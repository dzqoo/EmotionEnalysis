...
def save_keys(keys_truple, global_weight, save_path):
    '''the format of the key :keys,index,frequant'''
    wordcut = []
    # Infile = "F:/new2/ForTrain/SVM/tmsvm_for_win_1.2.0/data/deletetitle.key"
    Infile = '../data/model1/model/title1cut.key'
    c = 0
    with codecs.open(Infile, "r", encoding="utf-8") as fin:
        for line in fin:
            c += 1
            strline = line.strip("\n").encode("utf-8")
            wordcut.append("%s" % strline)
    print("c:"+str(c))
    d = 0
    '''
    for i in wordcut:
    ...
    for truple in keys_truple:
        count += 1
        # print truple[0].strip()
        if truple[0].strip() in wordcut:
            # print truple[0].strip()
            # print type(truple[0].strip())
            d += 1
            continue
        f.write(truple[0].strip() + "\t" + str(count) + "\t" + str(global_weight[truple[0].strip()]) + "\n")
    print ("d:"+str(d))
