# -*- coding: utf-8 -*-
"""
Created on Thu Dec 01 18:02:38 2016
...
print (u'read data ...')
train_data = pd.read_csv('./data/TrainSecond.csv', index_col='SentenceId', delimiter='\t', encoding='utf-8')
test_data = pd.read_csv('./data/Test.csv', index_col='SentenceId', delimiter='\t', encoding='utf-8')
train_label = pd.read_csv('./data/LabelSecond.csv', index_col='SentenceId', delimiter='\t', encoding='utf-8')
...
word2id = word2id[word2id >= min_count]  # 取出Series中元素个数大于min_count的元素
...
def gen_ngrams(s):
    s = [0] * (n - 1) + s + [0] * (n - 1)
    return zip(*[s[i:] for i in range(n)])  # 压缩解压
...
from keras.layers import Input, Embedding, GRU, Dense, TimeDistributed, Bidirectional, Convolution1D, ZeroPadding1D
...
ner_cnn = Convolution1D(64, 3)(ZeroPadding1D(1)(ner_embedded))
ner_brnn = Bidirectional(RNN(32, return_sequences=True), merge_mode='sum')(ner_cnn)
...
nb_epoch = 300
batch_size = 1024
ner_history_1 = ner_model.fit(ner_data, ner_target, batch_size=batch_size, nb_epoch=nb_epoch,
...
ner_data = np.array(list(ner_data))
ner_target = train_data['label'].append(test_data['label']).apply(list).apply(
    lambda s: s[:maxlen] + [0] * (maxlen - len(s[:maxlen])))
ner_target = np.array(list(ner_target))
ner_target = np.array(map(lambda y: np_utils.to_categorical(y, 5), ner_target))
